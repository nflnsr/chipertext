...
import { encrypt, decrypt } from "../utils/AutoKeyVigenereChiper";
import { useNavigate } from "react-router-dom";

function VariantVigenere() {
  ...
  const handleEncrypt = () => {
    const result = encrypt(text.toUpperCase(), key);
    setResult(result);
  };

  const handleDecrypt = () => {
    const result = decrypt(text.toUpperCase(), key);
    setResult(result);
  };
    ...
    <>
      <main className="h-[calc(100svh)]">
        ...
        <div className="absolute w-56 space-y-3 text-center -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
          <h1 className="text-lg font-semibold text-gray-700 underline">Variant Vigenere Cipher</h1>
          <input
            type="text"
            value={text}
